...
elsevier_pdf_download <- function(elsevier_xml_link, filepath) {
  ...
  json_obj <- scidir_html_link[2] %>% # we need to index to the second link in the `scidir_html_link` obj
    xml2::read_html() %>% # get html response from science direct link retrieved from elsevier XML response
    xml2::html_nodes('body') %>% # start from the body section
    xml2::html_nodes('div') %>% # get the div contents within that body section
    xml2::html_nodes('script') %>% # get the script contents within that div
    xml2::html_text() %>% # extract text (JSON string)
    jsonlite::fromJSON() # convert JSON string to R obj
  ...
  read_html(url_of_interest) %>%
    xml2:: html_node('head') %>% # start in the head section of the html
    xml2::html_nodes('meta[http-equiv="Refresh"]') %>% # from there, seek a meta tag with attribute http-equiv="Refresh"
    xml2::html_attr('content') %>% # within that tag, grab the character string containing the PDF direct access url
